...
class Preispirat(Scraper):
    ...
    def login(self):
        ...
        if (len(self.driver.find_elements_by_class_name('user-ava-intop'))>0): return
    ...
    def uploadProduct(self, product):
        ...
        if isinstance(toppreis, bool):
            print("returned")
            return
        if self.enter_info(toppreis) != None: return
        #self.driver.find_element_by_name('Deal einsenden').click()

    def get_toppreis(self, product):
        print(f'https://www.toppreise.ch/produktsuche?q={str(product.product.manufacturer_id).replace(" ", "+")}')
        user_input = None
        while(True):
            try:
    ...
    def enter_info(self, toppreis):
        ...
        description = f'{toppreis.product.product.name} ist derzeit bei {toppreis.product.company.name} für nur CHF {toppreis.topprice} erhältlich.\n\nZweitbester Preis ist laut Toppreise.ch CHF {toppreis.secondprice}: {toppreis.url}'
        ...
        self.driver.find_element_by_id('wpfepp-form-1-rehub_offer_product_price-field').send_keys('CHF ' + str(toppreis.topprice) + '.-')
        self.driver.find_element_by_id('wpfepp-form-1-rehub_offer_product_price_old-field').send_keys('CHF ' + str(toppreis.secondprice) + '.-')
        ...
        #while len(self.driver.find_elements_by_xpath('./div[contains(@class, "wpfepp-thumbnail-container")]')) < 1:
